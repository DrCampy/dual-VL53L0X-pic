...
*		utilized new Flash page/row definitions from device header files, 
*		and skipped all odd Flash word locations on devices with ECC
************************************************************************/

#include <xc.h>
#include "../Include/DEE Emulation 16-bit/DEE Emulation 16-bit.h"
...
void ErasePage(unsigned char bank, unsigned char page)
{
    ...
    #ifdef _ERASE    // For some devices, we need to do a dummy table write to the page to be erased
        TBLPAG = DEE_PAGE_TBL(bank, page);
        WritePMLow(pmOffset, pmOffset);
    #else            // For other devices, we need to point NVMADRU/NVMADR to the page to be erased
        NVMADRU = DEE_PAGE_TBL(bank, page);
        NVMADR = pmOffset;
    #endif
...
int PackEE(unsigned char bank)
{
    ...
    // For dsPIC33E/PIC24E devices only, do not use row writes, as many devices only support double-word writes
    NVMCON = PROGRAM_WORD;		
    
    WritePMLow(0xFFFF, packedOffset);
    WritePMHigh(0xFF, packedOffset);
    #ifdef __HAS_ECC
	packedOffset += 4;		// Do not use odd Flash words on dsPIC33E devices with ECC
    #else
	packedOffset += 2;		// Use both even and odd Flash words on devices without ECC
    #endif
    latchAddr = 0;

    do
    {
        latchData = DataEERead((DATA_EE_SIZE * bank) + latchAddr);
	...
	#ifdef __HAS_ECC
	    packedOffset += 4;		// Do not use odd Flash words on devices with ECC
	#else		
            packedOffset += 2;
	#endif	
        }
    ...
    TBLPAG = DEE_PAGE_TBL(bank, packedPage);
    #ifdef __HAS_ECC
	packedOffset = DEE_PAGE_OFFSET(bank, packedPage) + 4;	// Adjust offset to skip the odd word
    #else    
    ...
    {
        if(DataEERead((DATA_EE_SIZE * bank) + latchAddr) != latchData)
        {
            TBLPAG = savedTBLPAG;
            SetPageWriteError(1);
...
unsigned int DataEERead(unsigned int addr)
{
    ...
    // Point to proper TBLPAG and offset
    TBLPAG = DEE_PAGE_TBL(bank, currentPage);
    #ifdef __HAS_ECC        // Skip over preceding odd address in devices with ECC
        pmOffset = DEE_PAGE_OFFSET(bank, (currentPage + 1)) - 4;
    #else
	...
	#ifdef __HAS_ECC
	    pmOffset -= 4; 	// Skip over preceding odd address in devices with ECC
            i-=2;
	#else	
            pmOffset -= 2;
            i--;
    ...
    #ifdef __HAS_ECC
        pmOffset += 4;				// Adjust offset to skip the odd word
    #else
        pmOffset += 2; 
    #endif
